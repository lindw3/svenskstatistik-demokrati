...
# Karta
data <- st_read("C:\\Users\\WILIDF17\\OneDrive - Sveriges Riksidrottsförbund\\Dokument\\GitHub\\Svensk statistik\\svenskstatistik-demokrati\\data\\RegSO_2018_v2.gpkg" , layer = "RegSO_2018_v2")
kommuner_karta <- data %>%
...
# Politiskt engagemang efter utbildningsnivå
...
# Politiskt engagemang efter inkomst
...
# Politiskt engagemang efter bakgrund
...
### Om rapporten
I denna rapport bemöts temat **Demokrati** utifrån dessa områden:
<br><br>
...
- **Förtroendevalda**: Förtroendevalda utifrån kön, ålder, utbildningsnivå, inkomst och bakgrund. <br>
- **Politiskt engagemang**: Politiskt engagemang utifrån kön, ålder, utbildningsnivå, inkomst och bakgrund.
<br> <br>
...
df_combined %>% 
  ...
  ggplot(aes(x = År , y = Blocktillhörighet , group = Ålder , color = Ålder,
             text = paste(År, "\nKategori: " , Ålder, "\nAndel höger: " , Blocktillhörighet * 100, "%"))) +
  geom_line(linewidth = 1) +  xlab(NULL) +
  scale_y_continuous(limits = c(0, 1), 
                     breaks = 0.5, 
                     labels = scales::percent_format(accuracy = 1)) +
...
# Politiskt engagemang efter kön {data-navmenu="Politiskt engagemang"}
## {.tabset .tabset-pills}
### Politiskt engagemang efter kön
```{r}
...
df %>% 
  ggplot(aes(x = factor(Indikator , levels = levels_politik) , y = Andel/100 , fill = Kön,
  ...
  ylab("Andel, %") + xlab(NULL) +
  scale_fill_manual(values = c("Kvinnor" = "#ffb1b1",
                               "Män" = "#64b6ee"))    -> p_politisktengagemang_kön

ggplotly(p_politisktengagemang_kön, tooltip = "text") %>%
  config(displayModeBar = F)
```


# Politiskt engagemang efter ålder {data-navmenu="Politiskt engagemang"}
## {.tabset .tabset-pills}
### Politiskt engagemang efter ålder
```{r}
df <- politisktengagemang_ålder_kön %>% 
  pivot_longer(!c(Indikator, Ålder, Kön) , names_to = "År" , values_to = "Andel") %>% 
  filter(Ålder != "Totalt" , 
...
df %>% 
  ggplot(aes(x = factor(Indikator , levels = levels_politik) , y = Andel/100 , fill = Ålder,
             text = paste(Indikator, "\nÅlderskategori: " , Ålder, "\nAndel: " , Andel , "%"))) +
  geom_col(position = "dodge") +  
  ss_theme +
  theme(axis.text.x = element_text(angle = 70, hjust = 1 , vjust = 1)) +
  ...
  scale_fill_manual(values = c("16-29 år" = "#ffb1b1",
                               "30-49 år" = "#64b6ee" ,
                               "50-64 år" = "#94c58c",
                               "65+ år" = "#ffff9a"))    -> p_politisktengagemang_ålder

ggplotly(p_politisktengagemang_ålder, tooltip = "text") %>%
  config(displayModeBar = F)
```
...
# Politiskt engagemang efter utbildning {data-navmenu="Politiskt engagemang"}
## {.tabset .tabset-pills}
### Politiskt engagemang efter utbildning
```{r}
...
df %>% 
  ggplot(aes(x = factor(Indikator , levels = levels_politik) , 
             y = Andel/100 , fill = factor(Utbildning, levels = levels) ,
             text = paste(Indikator, "\nUtbildningsnivå: " , Utbildning, "\nAndel: " , Andel , "%"))) +
  geom_col(position = "dodge") +  
  ss_theme +
  theme(axis.text.x = element_text(angle = 70, hjust = 1 , vjust = 1)) +
  ...
  scale_fill_manual(values = c("Förgymnasial utbildning" = "#ffb1b1",
                               ...
                               "Eftergymnasial utbildning, 3 år eller längre" = "#ffff9a"))    -> p_politisktengagemang_utbildning

ggplotly(p_politisktengagemang_utbildning, tooltip = "text") %>%
  config(displayModeBar = F)
```



# Politiskt engagemang efter inkomst {data-navmenu="Politiskt engagemang"}
## {.tabset .tabset-pills}
### Politiskt engagemang efter inkomst
```{r}
...
df %>% 
  ggplot(aes(x = factor(Indikator , levels = levels_politik) , y = Andel/100 , fill = Inkomst,
             text = paste(Indikator, "\nInkomst: " , Inkomst, "\nAndel: " , Andel , "%"))) +
  geom_col(position = "dodge") +  
  ss_theme +
  theme(axis.text.x = element_text(angle = 70, hjust = 1 , vjust = 1)) +
  ...
  scale_fill_manual(values = c("1 kvintilen" = "#ffb1b1",
                               ...
                               "3 kvintilen" = "#94c58c",
                               "4 kvintilen" = "#ffff9a" ,
                               "5 kvintilen" = "#ffcff1"))    -> p_politisktengagemang_inkomst

ggplotly(p_politisktengagemang_inkomst, tooltip = "text") %>%
  config(displayModeBar = F)
```



# Politiskt engagemang efter bakgrund {data-navmenu="Politiskt engagemang"}
## {.tabset .tabset-pills}
### Politiskt engagemang efter bakgrund
```{r}
...
df %>% 
  ...
  scale_fill_manual(values = c("Utländsk bakgrund" = "#ffb1b1",
                               "Svensk bakgrund" = "#64b6ee"))    -> p_politisktengagemang_bakgrund

ggplotly(p_politisktengagemang_bakgrund, tooltip = "text") %>%
  config(displayModeBar = F)
```
